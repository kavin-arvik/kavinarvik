<template>
  ...
  export default {
    ...
    data() {
      return {
        currentDate :new Date(new Date().setMonth(new Date().getMonth())).toJSON().slice(0, 10),
        userArray: [],
    ...
    getUser(){
      for (let i=0; i<this.login.length; i++){
        ...
        if(this.login[i].role == 'biller'){
          this.userArray.push(login[i].userid)
          console.log("2");
        }
      }
    },
  mapping(){
    for(let i=0; i<this.userArray.length; i++){
      ...
      for(let j=0; j<this.billmaster.length; j++){
        if(this.userArray[i] == this.billmaster[j].userid && this.billmaster[j].BillDate == this.currentDate){
